...
# Passo a passo — Domínios próprios

Por padrão, o EasyPanel expõe cada serviço em um domínio genérico gerado automaticamente (ex: `ultragas-ultragas-n8n.tlwieg.easypanel.host`). Para atender ao item do roadmap que exige acesso sempre via domínio HTTPS próprio, os dois serviços do cliente `ultragas` foram migrados para subdomínios do domínio principal do negócio.

## 1. Criação dos registros DNS

Na Cloudflare, foram criados dois registros `A`, seguindo o mesmo procedimento da Fase 8 (DNS only temporariamente, para validação do certificado):

```
ultragas-n8n.nexflowdx.cloud        → IP da VPS
ultragas-evolution.nexflowdx.cloud  → IP da VPS
```

## 2. Atualização do Host em cada serviço

Em **Domínios**, dentro de cada serviço (`ultragas-n8n` e `ultragas-evolution`), o domínio automático existente foi editado (não recriado), substituindo o campo **Host** pelo novo domínio, mantendo os campos de destino (protocolo, porta, caminho) inalterados — o EasyPanel já direciona corretamente para a porta interna correspondente (5678 no n8n, 8080 na Evolution API).

### Objetivo

Reaproveitar a configuração de proxy reverso já existente, alterando apenas o endereço público pelo qual o serviço é acessado, sem impactar o roteamento interno do container.

## 3. Validação

Ambos os domínios foram testados com sucesso após a propagação:

- `https://ultragas-n8n.nexflowdx.cloud` — editor do n8n acessível normalmente
- `https://ultragas-evolution.nexflowdx.cloud` — retorno da rota raiz da API (`status: 200`) e painel visual acessível em `/manager`

### Objetivo

Confirmar emissão automática do certificado Let's Encrypt para os novos domínios e funcionamento correto do roteamento antes de considerar a fase concluída.

---

# Limpeza de recursos duplicados

Assim como ocorreu com o n8n, o template da Evolution API criou automaticamente um banco de dados e uma instância de Redis próprios (`ultragas-evolution-db` e `ultragas-evolution-redis`), não utilizados, já que a instância foi reconectada ao `ultragas-postgres` e ao `ultragas-redis` existentes.

Ambos os serviços duplicados foram excluídos. Após a exclusão, o acesso ao `ultragas-evolution` (incluindo a tela de login do Manager) foi validado novamente, confirmando que a instância dependia exclusivamente da infraestrutura correta, sem qualquer referência residual aos recursos removidos.
...
✅ Domínios próprios configurados e validados: `ultragas-n8n.nexflowdx.cloud` e `ultragas-evolution.nexflowdx.cloud`, ambos com certificado HTTPS emitido automaticamente
✅ Recursos duplicados do template da Evolution API (`ultragas-evolution-db`, `ultragas-evolution-redis`) identificados e removidos, com validação de que o serviço principal continuou funcionando normalmente após a exclusão
...
- Migrar um serviço já em funcionamento para um domínio próprio é uma operação de baixo risco quando feita editando o domínio existente (em vez de recriar do zero) e mantendo os parâmetros de destino (porta, protocolo, caminho) inalterados.
- Revalidar o funcionamento de um serviço logo após remover recursos identificados como duplicados é a forma mais confiável de confirmar que a dependência foi realmente eliminada, e não apenas presumida.
